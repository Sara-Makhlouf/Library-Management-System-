...
function RoleChip({ role }) {
  ...
  return (
    <Box sx={{
      ...
      ...(isAdmin
        ? { bgcolor: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", color: GOLD_DARK }
        : { bgcolor: "rgba(127,119,221,0.1)", border: "1px solid rgba(127,119,221,0.2)", color: "#6a61d1" }),
    }}>
      {role}
    </Box>
  );
  ...
  return (
    <Box sx={{
      ...
      ...(active
        ...
        : { bgcolor: goldA(0.06), border: `1px solid ${BORDER}`, color: MUTED }),
    }}>
    ...
    <Box sx={{ mx: "16px", mb: "14px" }}>
      <Box sx={{
        ...
        <Box sx={{
          ...
          <Box>
            <Typography sx={{ ...display, fontSize: 13, fontWeight: 600, color: GOLD_DARK, mb: "2px" }}>
              {p.name}
            </Typography>
          ...
          <Box sx={{
            ...
            fontSize: 10, fontWeight: 600, color: GOLD_DARK,
          }}>
            ⭐ {p.points} pts
          </Box>
        </Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)" }}>
          {stats.map(({ label, value, gold }, i) => (
            <Box key={label} sx={{
              px: "16px", py: "12px",
              borderRight: i < 3 ? `1px solid ${goldA(0.1)}` : "none",
            }}>
              <Typography sx={{ ...mono, fontSize: 10, color: MUTED2, mb: "5px" }}>
                {label}
              </Typography>
              <Typography sx={{ ...display, fontSize: 15, fontWeight: 700, color: gold ? GOLD_DARK : TEXT }}>
                {value}
              </Typography>
            </Box>
          ))}
    ...
    <Box sx={{ minHeight: "100vh", bgcolor: BG, p: { xs: 2, md: "20px 24px" }, fontFamily: "Inter, sans-serif" }}>
      ...
      <Box sx={{ bgcolor: SURFACE, border: `1px solid ${BORDER}`, borderRadius: "18px", overflow: "hidden" }}>
        ...
        {userList.length === 0 ? (
          ...
          return (
            <Box key={u.id} sx={{ borderBottom: i < userList.length - 1 ? `1px solid ${goldA(0.1)}` : "none" }}>
              ...
              <Box
                ...
                sx={{
                  ...
                  bgcolor: isOpen ? "rgba(201,168,76,0.05)" : "transparent",
                  transition: "background .15s",
                  "&:hover": { bgcolor: isOpen ? "rgba(201,168,76,0.06)" : goldA(0.05) },
                }}
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: "10px", minWidth: 0 }}>
                  ...
                  <Box sx={{
                    fontSize: 11, color: isOpen ? GOLD_DARK : MUTED2,
                ...
                <Box sx={{ display: "flex", justifyContent: "center" }}>
                  <Box
                    ...
                    sx={{
                      ...
                      bgcolor: isOpen ? "rgba(201,168,76,0.14)" : "rgba(201,168,76,0.07)",
                      color: GOLD_DARK, transition: "all .2s",
                      "&:hover": { bgcolor: "rgba(201,168,76,0.18)", borderColor: "rgba(201,168,76,0.5)" },
                    }}
                  >
                    {isThisLoading
                  ...
                  <Tooltip title="Delete user" arrow>
                    <IconButton
                      ...
                      sx={{
                        ...
                        border: `1px solid ${BORDER}`,
                        bgcolor: goldA(0.04),
                        color: "rgba(200,67,61,0.7)",
                        "&:hover": { bgcolor: "rgba(248,113,113,0.12)", borderColor: "rgba(248,113,113,0.35)", color: "#c8433d" },
                        transition: "all .2s",
                      }}
                    >
                      <Trash2 size={15} />
      ...
      >
        <DialogContent sx={{ p: "28px 26px 8px" }}>
          <Box sx={{
            ...
            <TriangleAlert size={20} color="#c8433d" />
          </Box>
