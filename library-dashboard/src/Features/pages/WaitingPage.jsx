...
import {
  ...
  getTopWaitingList,
} from "../../Core/Redux/Thunks/WaitingListThunk";
...
export default function WaitingListPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        await Promise.all([
          dispatch(getBooksInvaliable()).unwrap(),
          dispatch(getTopWaitingList()).unwrap(),
        ]);
      } catch (error) {
        if (!cancelled) {
  ...
  const refreshData = async () => {
    try {
      ...
      console.error("Failed to refresh waiting list:", error);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await dispatch(
        deleteFromWatingList(deleteEntry.id)
      ).unwrap();
      ...
      console.error(
        "Failed to delete waiting list entry:",
        err
      );
    } finally {
      setDeleting(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {pagination?.total ?? items.length} waiting
        </Box>
      </Box>
      ...
      >
        <Box
          sx={{
            p: { xs: 3, md: "32px 36px" },
            borderRadius: "20px",
            background: `linear-gradient(160deg, ${goldA(
              0.14
            )}, ${goldA(0.04)})`,
        ...
        >
          ...
          >
            Ordered by who's been waiting longest.
            Remove an entry once it's resolved.
          </Typography>
        </Box>
        ...
        {topItems.length > 0 && (
          ...
          >
            ...
            {topItems.slice(0, 4).map((top, i) => {
              const maxCount =
                topItems[0]?.waiting_count || 1;
                ...
                >
                  ...
                  >
                    <Box
                      ...
                      src={
                        entry.book?.cover
                          ? `${IMAGE_BASE_URL}${entry.book.cover}`
                          : "/history book.jpg"
                      }
                      onError={(e) => {
                        e.currentTarget.src =
      ...
      <Dialog
        ...
        PaperProps={{
          sx: {
            ...
            bgcolor: SURFACE,
            border:
              "1px solid rgba(224,101,95,0.22)",
            boxShadow:
              "0 24px 60px rgba(224,101,95,.16)",
          },
        }}
      >
        <DialogContent
        ...
        >
          ...
          >
            They'll need to rejoin the list if
            they're still interested in this book.
          </Typography>
          ...
          {deleteEntry && (
            ...
            >
              <Box
                ...
                loading="lazy"
                src={
                  deleteEntry.book?.cover
                    ? `${IMAGE_BASE_URL}${deleteEntry.book.cover}`
                    : "/history book.jpg"
                }
                onError={(e) => {
                  e.currentTarget.src =
        ...
        >
          ...
          <Button
            ...
            sx={{
              ...
              color: "#fff",
              background:
                "linear-gradient(135deg,#e0655f,#a83a30)",
              "&:hover": {
                background:
                  "linear-gradient(135deg,#e87c76,#b8483d)",
                boxShadow:
                  "0 12px 28px rgba(224,101,95,0.25)",
              },
              "&.Mui-disabled": {
                color:
